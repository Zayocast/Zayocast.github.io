...
(function () {
  ...
  TERA.views.order = {
    ...
    render: function () {
      ...
      UI.card({
        title: 'Малки гаранции',
        desc: 'Редът с отметки под стъпките. Кратки обещания, които свалят колебанието.',
        body: UI.fields([
          { k:'badges', label:'Гаранции', type:'tags', w:'full',
            hint:'Кратко, по 2–4 думи: Без предплащане, Плащаш на място… Enter добавя нова.' }
        ], 'order')
      }) +
      ...
      UI.card({
        ...
        body: UI.fields([
          ...
          { k:'timeChips',       label:'Бързи бутони за час', type:'lines', w:'full', rows:4,
            hint:'По един на ред. Клиентът натиска и часът се попълва сам.' }
        ], 'order')
      }) +

      UI.card({
        title: 'Връзка с кошницата',
        desc: 'Розовата лента най-горе във формата, когато клиентът вече си е направил списък.',
        body: UI.fields([
          { k:'cartHint', label:'Какво го пита', type:'text', w:'full',
            ph:'В списъка ти има продукти — да ги сложа ли в поръчката?' },
          { k:'cartBtn',  label:'Текст на бутона', type:'text', ph:'Добави списъка' }
        ], 'order')
      }) +
