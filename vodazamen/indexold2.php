...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    foreach ($orders as $index => $row) {
        ...
        ?>
        <tr id="order_<?php echo $row['order_id']; ?>" class="hover:bg-gray-50 transition" data-order-date="<?php echo $order_date; ?>" data-address="<?php echo htmlspecialchars($row['address'] ?? ''); ?>" data-address-details="<?php echo htmlspecialchars($row['address_details'] ?? ''); ?>" data-total-price="<?php echo $row['total_price']; ?>">
            ...
            <td class="p-3 border-b border-r text-center">
                <form method="POST" class="inline">
                    ...
                    <select name="status" class="border p-2 rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 transition w-full status-select" onchange="this.form.submit()">
...
<body class="bg-gradient-to-r from-gray-100 to-gray-200">
    ...
    <form id="printForm" method="POST" action="print_orders.php" target="_blank" style="display: none;">
        <input type="hidden" name="order_ids" id="printOrderIds">
    </form>
        ...
        $(document).ready(function() {
            ...
            function setupEventListeners() {
                ...
                $(window).on('click', function(event) {
                    if ($(event.target).is('#editModal') || $(event.target).is('#commentModal')) {
                        $('#editModal').hide();
                        $('#commentModal').hide();
                    }
                });
            ...
            $('#print_selected').on('click', function() {
                const selectedOrderIds = $('.order_checkbox:checked').map(function() {
                    return $(this).data('id');
                }).get();
                if (selectedOrderIds.length > 0) {
                    $('#printOrderIds').val(JSON.stringify(selectedOrderIds));
                    $('#printForm').submit();
                }
            });
        });
